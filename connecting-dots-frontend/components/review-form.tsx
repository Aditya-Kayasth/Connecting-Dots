...
export function ReviewForm({ applicationId, recipientName }: { applicationId?: string; recipientName?: string }) {
  const searchParams = useSearchParams()
  const resolvedAppId = applicationId || searchParams?.get("applicationId") || "app-2048"
  const resolvedRecipient = recipientName || searchParams?.get("recipientName") || "Partner"
  ...
  async function submitReview(event: React.FormEvent) {
    ...
    try {
      await apiRequest("/api/v1/core/reviews", {
        method: "POST",
        body: {
  ...
  return <form onSubmit={submitReview} className="review-form">
    <div><span className="eyebrow">Completed application</span><h2>How was working with {resolvedRecipient}?</h2><p className="muted">Your review will appear on their public profile.</p></div>
