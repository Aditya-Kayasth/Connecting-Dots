...
- **Cold-start resilience across the chain:** because a request now hops `gateway → core/ai`, a single sleeping downstream service fails the whole request rather than showing Render's browser loading page (that only applies to direct browser hits). Add a Resilience4j retry + circuit breaker to `gateway-service`'s routes with a 60–90s timeout budget and 2–3 backoff retries, so a cold downstream service degrades to "slow first request" instead of a hard error.
- **Don't try to keep all four services warm 24/7 with a ping trick** — Render's 750 free instance-hours/month are shared per workspace, not per service; four services always-on would need ~2,880 hours/month, well over budget, and would suspend your free services for the rest of the month. If you want pre-demo warmth, either scope a keep-alive ping (cron-job.org/UptimeRobot free tier) to a specific window, or just manually hit each `/health` endpoint a minute or two before showing the live link.
